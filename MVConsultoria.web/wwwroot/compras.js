...
// Função para filtrar compras pelo nome do cliente
function filtrarComprasPorCliente() {
    const query = document.getElementById('searchClienteField').value.toLowerCase();
    
    // Filtra as compras com base no nome do cliente
    const comprasFiltradas = compras.filter(compra => 
        compra.nomeCliente.toLowerCase().includes(query)
    );

    exibirCompras(comprasFiltradas); // Atualiza a exibição com as compras filtradas
}

// Adiciona o evento de input no campo de busca
document.getElementById('searchClienteField').addEventListener('input', filtrarComprasPorCliente);
*/
...
function filtrarCompras() {
    ...
    const comprasFiltradas = compras.filter(compra => {
        ...
        const dataMatch = dataQuery ? new Date(compra.dataCompra).toISOString().split('T')[0] === dataQuery : true;
...
document.getElementById('saveNewCompraBtn').addEventListener('click', async function () {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert(`Erro: ${errorData.message || 'Falha ao cadastrar a compra.'}`);
        }
    } catch (error) {
...
async function mostrarModalVerParcelas(compraId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Limpa a lista de parcelas do modal
            const parcelasList = document.getElementById('parcelas-list');
            parcelasList.innerHTML = '';

            // Preenche o modal com as parcelas recebidas
            parcelas.forEach(parcela => {
                const listItem = document.createElement('li');
                listItem.innerHTML = `
                    <strong>Parcela ID:</strong> ${parcela.id} <br>
                    <strong>Data de Vencimento:</strong> ${new Date(parcela.dataVencimento).toLocaleDateString()} <br>
                    <strong>Valor:</strong> R$ ${parcela.valor.toFixed(2)} <br>
                    <strong>Pago:</strong> ${parcela.pago ? 'Sim' : 'Não'} <br>
                    ${parcela.pago ? `<strong>Data de Pagamento:</strong> ${new Date(parcela.dataPagamento).toLocaleDateString()} <br><strong>Valor Pago:</strong> R$ ${parcela.valorPago.toFixed(2)}` : ''}
                `;
                parcelasList.appendChild(listItem);
            });
